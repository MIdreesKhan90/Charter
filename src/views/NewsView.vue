...
<template>
  <section class="section-title">
    <div class="container">
      <h1 className="section-title-header">News</h1>
  ...
  <section class="blog-slider-section">
    ...
    >
      <template v-for="(item, i) in articleStore.articles">
        <swiper-slide v-if="articleStore.isArticlePromoted(item.id, 'news')" :key="i">
          <div
            class="slide-item"
            :style="{
          ...
          >
            <RouterLink :to="'/article/' + item.url" class="about-blog">
              ...
              <div class="social-status" v-if="item.writer">
                <img :src="commonStore.publicData(`/articles/${item.id}.writer.png`)" :alt="item.writer" loading="lazy"/>
                <div class="about">
                  <h5>{{ item.writer }}</h5>
                </div>
              </div>
